...
<div class="card shadow mb-4">
  ...
  <div class="card-header py-3">
      <div class="row">
          ...
          <div class="col-sm-4"> 
          <h6 class="m-0 font-weight-bold text-info col-10">Tous les médicaments</h6> 
          </div>
        ...
        <input  name="rechercheN" id="rechercheN" oninput="fonction2()" class="form-control" autocomplete="off"        placeholder= "Nom Du Médicament...">
          ...
          <div class="col-sm-2" > 
           <a href="/medicament/index" class="btn btn-info ">Ajouter médicament</a>
          </div>  
  ...
  <div class="card-body">
      ...
      @if (session()->has('InsertSuccessM'))
      <div class="alert alert-success alert-dismissible" >
           <strong>Opération réussie: </strong>
          {{session()->get('InsertSuccessM')}}
          <button class = "close btn-success" data-dismiss="alert" > × </button>
      </div>
      @endif
      @if (session()->has('ExisteErrorM'))
      <div class="alert alert-danger alert-dismissible" >
          <strong>Avertissement: </strong>
          {{session()->get('ExisteErrorM')}} 
          <button class = "close btn-danger" data-dismiss="alert" > × </button>
      </div>
      @endif
      @if (session()->has('UpdateSuccessM'))
      <div class="alert alert-success alert-dismissible" >
          <strong>Opération réussie: </strong>
          {{session()->get('UpdateSuccessM')}}
          <button class = "close btn-success" data-dismiss="alert" > × </button>    
      </div>
      @endif
      @if (session()->has('DeleteSuccessM'))
      <div class="alert alert-success alert-dismissible" >
          <strong>Opération réussie: </strong>
          {{session()->get('DeleteSuccessM')}} 
          <button class = "close btn-success" data-dismiss="alert" > × </button>
      </div>
      @endif
